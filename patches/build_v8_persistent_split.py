...
BASE = Path('/mnt/data/BioRevHD 30-Enero-2013 LOCAL COOP v7 FULLPAD NATIVE SPLIT.exe')
...
OUT = Path('/mnt/data/BioRevHD 30-Enero-2013 LOCAL COOP v8 FULLPAD PERSISTENT SPLIT.exe')
MAN = Path('/mnt/data/rev1_local_coop_research/p2-fullpad-persistent-split-v8-manifest.json')

EXPECTED_BASE_SHA='fe12f35c1d08f61961d7bdefba19a09e68939bb8966fc3d6c50e994ab699fa76'
TEXT_VA=0x01B79000
TEXT_RAW=0x400
...
manifest={
  ...
  'design':{
    ...
    'network_preservation':'ThinkMode::Network(3) never sets the local flag; stock camera exclusivity and viewport behavior are preserved when flag=0',
  ...
  'helper':{
    'base_va':hex(HELPER_BASE),'size':len(blob),'end_va':hex(HELPER_BASE+len(blob)),
    'entries':{
      'local_enable':'0x01C94FC0','pad_rebind_check':'0x01C94FD4','conditional_deactivate':'0x01C94FF8',
      'partner_view_index':'0x01C9500C','conditional_bit0':'0x01C95020','conditional_bit1':'0x01C95036','ensure_split':'0x01C9504C'
    }
  },
  'diff':{
    'same_file_size':True,
  ...
  'limitations':[
    ...
    'The patch statically guarantees a distinct Partner camera target and persistent activation/binding when the local flag is active, but rendering correctness, collision/culling and cutscene transitions must be tested in-game.',
    'HUD, inventory/pause ownership, scripted interactions and QTE behavior remain unresolved.',
    'The second physical controller still requires runtime confirmation that Windows/DInput populates sPad::Pad[1] as expected on the user setup.'
  ],
